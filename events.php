<div id="event-read-more-block">
    ...
    <div id="event-right">
        <img id="event-img-01" class="event-row-01" src="images/product_01.jpg" alt="Hier komt een afbeelding van een festival.">
        <img id="event-img-02" class="event-row-02" src="images/product_01.jpg" alt="Hier komt een afbeelding van een festival.">
    </div>
</div>
	...
	<script>
            ...
            $('#event-img01, #event-img02').load( function() {
        ...
        function readMore(){
            if(expanded == false){
                ...
                $("#event-left, #event-right").animate({
                    ...
                    // animation done
                    $("#event-left .event-row-02").append('<span id="event-expand" style="cursor: pointer; color: rgba(109,207,246,1)" onclick="readMore();"></span>');
                    $("#event-expand").html(read_more_text_closed);
                    expanded = true;
                });
                $('body, html').animate({scrollTop: ($('#event-read-more-block').offset().top + $('#event-read-more-block').outerHeight(true)) - $(window).outerHeight(true) + 75}, 500);
                $('#event-expand').remove();
            }else{
